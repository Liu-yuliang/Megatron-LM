...
PYTHIA_SIZE="${PYTHIA_SIZE:-160m}"
RUN_NAME="${RUN_NAME:-pythia-${PYTHIA_SIZE}-${JOB_ID:-local}-$(date +%Y%m%d_%H%M%S)}"
RUN_DIR="${RUN_DIR:-$RUN_ROOT/$RUN_NAME}"
MODEL_ROOT="${MODEL_ROOT:-/mnt/shared-storage-gpfs2/plm-gpfs/public_data_model/hnet/model}"
HF_MODEL_DIR="${HF_MODEL_DIR:-$MODEL_ROOT/pythia-$PYTHIA_SIZE}"
...
export TMPDIR="${TMPDIR:-/tmp/public_pythia_${JOB_ID:-local}_$$}"
export TMP="$TMPDIR"
export TEMP="$TMPDIR"
...
load_pythia_config() {
  case "$PYTHIA_SIZE" in
    ...
    6.9b)
      ...
      ;;
    12b)
      VOCAB_SIZE="${VOCAB_SIZE:-50688}"
      NUM_LAYERS="${NUM_LAYERS:-36}"
      HIDDEN_SIZE="${HIDDEN_SIZE:-5120}"
      NUM_ATTENTION_HEADS="${NUM_ATTENTION_HEADS:-40}"
      FFN_HIDDEN_SIZE="${FFN_HIDDEN_SIZE:-20480}"
      ;;
    *)
      echo "Unsupported PYTHIA_SIZE=$PYTHIA_SIZE. Use one of: 160m, 410m, 1b, 1.4b, 2.8b, 6.9b, 12b." >&2
      exit 2
      ;;
  esac
  ...
  ROTARY_PERCENT="${ROTARY_PERCENT:-0.25}"
  NORM_EPSILON="${NORM_EPSILON:-1e-5}"
  INIT_METHOD_STD="${INIT_METHOD_STD:-0.02}"
  INIT_METHOD_VARIANT="${INIT_METHOD_VARIANT:-normal}"
  EOS_TOKEN_ID="${EOS_TOKEN_ID:-0}"
  PAD_TOKEN_ID="${PAD_TOKEN_ID:-0}"
  MAKE_VOCAB_SIZE_DIVISIBLE_BY="${MAKE_VOCAB_SIZE_DIVISIBLE_BY:-1}"
  KV_CHANNELS="${KV_CHANNELS:-$((HIDDEN_SIZE / NUM_ATTENTION_HEADS))}"
}
...
write_arch_summary() {
  python - "$HF_MODEL_DIR/config.json" "$RESULTS_DIR/pythia_arch_summary.txt" <<'PY'
import json, sys
...
build_data_args() {
  ...
  if [[ "$use_mock" == "1" || -z "${DATA_PATH:-}" ]]; then
    DATA_ARGS=(
      --mock-data
      --tokenizer-type NullTokenizer
      --vocab-size "$VOCAB_SIZE"
      --null-tokenizer-eod-id "$EOS_TOKEN_ID"
      --null-tokenizer-pad-id "$PAD_TOKEN_ID"
    ...
    DATA_ARGS=(
      ...
      --vocab-size "$VOCAB_SIZE"
      --data-cache-path "$DATA_CACHE_DIR/real"
      --split "${SPLIT:-99,1,0}"
      --no-create-attention-mask-in-dataloader
...
run_train() {
  ...
  local tp pp cp mbs gbs train_iters train_samples save_interval lr min_lr lr_decay_iters lr_decay_samples lr_warmup_iters lr_warmup_samples attention_backend
  tp="${TP_SIZE:-1}"
  pp="${PP_SIZE:-1}"
  cp="${CP_SIZE:-1}"
  ...
  if [[ -n "$train_iters" && -n "$train_samples" ]]; then
    echo "TRAIN_ITERS and TRAIN_SAMPLES are mutually exclusive" >&2
    exit 2
  fi
  if [[ -z "$train_iters" && -z "$train_samples" ]]; then
    train_iters=50
  fi
  save_interval="${SAVE_INTERVAL:-${train_iters:-1000}}"
  lr="${LR:-6.0e-4}"
  ...
  model_args=(
    ...
    --spec megatron.core.models.gpt.pythia_layer_specs pythia_parallel_residual_layer_spec
    ...
    --init-method-variant "$INIT_METHOD_VARIANT"
  )
  ...
  local manifest="$RESULTS_DIR/launch_manifest_${JOB_ID:-local}.txt"
  {
    ...
    echo "precision=bf16"
    echo "transformer_impl=transformer_engine"
    echo "init_method_variant=$INIT_METHOD_VARIANT"
    echo "layer_spec=megatron.core.models.gpt.pythia_layer_specs pythia_parallel_residual_layer_spec"
    ...
    echo "tp=$tp pp=$pp cp=$cp mbs=$mbs gbs=$gbs seq=$SEQ_LENGTH train_iters=$train_iters train_samples=$train_samples"
    echo "lr_decay_iters=$lr_decay_iters lr_decay_samples=$lr_decay_samples lr_warmup_iters=$lr_warmup_iters lr_warmup_samples=$lr_warmup_samples"
  ...
  log "Launching Pythia $PYTHIA_SIZE training via $TRAIN_ENTRYPOINT, log=$train_log"
  set +e
