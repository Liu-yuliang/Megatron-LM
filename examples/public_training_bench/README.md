...
W&B is local/offline by default: `WANDB_MODE=offline`, project `ConceptLM`, entity `iammi-nanjing-university`, and files under each run's `wandb/`, `wandb_cache/`, and `wandb_config/` directories. Set `ENABLE_WANDB=0` to disable it. With the current Megatron logger it records train loss keys, learning rate, batch size, grad norm, loss scale, iteration time, throughput TFLOP/s/GPU, `tps-per-gpu`, `mfu-h200-percent`, timers, and checkpoint artifacts when checkpoints are saved.
