...
"""Builder for the Megatron-Core ConceptLM V2.1 prototype."""

from megatron.core.models.gpt.conceptlm_v21 import ConceptLMV21Model
...
def conceptlm_v21_builder(
    ...
    """Build ConceptLM V2.1 without touching the V2 builder path."""

    print_rank_0(f"building ConceptLM V2.1 model with {args.conceptlm_backbone} backbone ...")
    if args.mtp_num_layers is not None:
        raise NotImplementedError("ConceptLM V2.1 and MTP are not wired together in this prototype")
    if config is None:
    ...
    if args.spec is not None:
        ...
        if args.transformer_impl != "transformer_engine":
            raise NotImplementedError(
                "ConceptLM V2.1 Pythia backbone currently expects transformer_engine"
            )
    ...
    return ConceptLMV21Model(
        ...
        concept_bottleneck_type=args.conceptlm_bottleneck_type,
        concept_vq_patch_ratio=args.conceptlm_vq_patch_ratio,
        concept_mlp_bottleneck_ratio=args.conceptlm_mlp_bottleneck_ratio,
        concept_mlp_bottleneck_activation=args.conceptlm_mlp_bottleneck_activation,
        concept_mlp_recon_loss_weight=args.conceptlm_mlp_recon_loss_weight,
        concept_mlp_hlm_loss_weight=args.conceptlm_mlp_hlm_loss_weight,
        concept_mlp_hidden_loss_weight=args.conceptlm_mlp_hidden_loss_weight,
        concept_mlp_hlm_loss_type=args.conceptlm_mlp_hlm_loss_type,
